...
static code unsigned long LunarCalendarTable[60] = {
  ...
  //  0x8A95BF,0x0A9553,0x0B4A47,0x6B553B,0x0AD54F,0x055A45,0x4A5D38,0x0A5B4C,0x052B42,0x3A93B6 /*2071-2080*/取消此段,节约代码空间
  // 0x069349,0x7729BD,0x06AA51,0x0AD546,0x54DABA,0x04B64E,0x0A5743,0x452738,0x0D264A,0x8E933E/*2081-2090*/取消此段,节约代码空间
  // 0x0D5252,0x0DAA47,0x66B53B,0x056D4F,0x04AE45,0x4A4EB9,0x0A4D4C,0x0D1541,0x2D92B5          /*2091-2099*/取消此段,节约代码空间
};
static code unsigned char jieqi[]=
{ 
...
{ 	uchar day=t->day;
	uchar temp;
	uint location=(t->year)*12+(t->mon)-1;
	    if(day<15)
        {
        temp=15-day;
        if((jieqi[location]>>4)==temp) return (t->mon*2-1);
        else return (0);
        }
        ...
        {
        temp=day-15;
        if((jieqi[location]&0x0f)==temp) return (t->mon*2);
        else return (0);
        }
    }
